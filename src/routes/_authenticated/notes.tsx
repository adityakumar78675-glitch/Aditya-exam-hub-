...
import { FileText, Search, Download, BookOpen, Loader2 } from "lucide-react";
...
export function CoverThumb({ note, className = "" }: { note: any; className?: string }) {
  const [failed, setFailed] = useState(false);
  const show = note.cover_url && !failed;
  return (
    <div className={`relative overflow-hidden rounded-xl bg-muted grid place-items-center ${className}`}>
      {show ? (
        <img
          src={note.cover_url}
        ...
        <div className="w-full h-full grid place-items-center bg-gradient-to-br from-primary/15 to-primary/5 text-primary">
          <FileText className="size-7" />
          <span className="text-[9px] font-semibold mt-1 tracking-wide">STUDY MATERIAL</span>
        </div>
      )}
    </div>
  );
...
function NotesPage() {
  ...
  const isPdf = (n: any) => (n.file_type || "").includes("pdf") || /\.pdf$/i.test(n.file_name || "");

  const openNote = async (n: any) => {
    try {
      setBusy(n.id);
      const url = await getSignedUrl(n);
      if (isPdf(n)) setViewer({ title: n.title, url });
  ...
  const downloadNote = async (n: any) => {
    try {
      ...
      supabase.rpc("increment_note_download", { _note_id: n.id }).then(() => {});
    } catch (e: any) { toast.error(e.message); } finally { setBusy(null); }
  };
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
          {filtered.map((n: any) => (
            <div key={n.id} className="bg-card border border-border rounded-2xl p-3 flex flex-col">
              <CoverThumb note={n} className="aspect-[3/4] w-full mb-3" />
              <p className="font-semibold text-sm leading-snug line-clamp-2">{n.title}</p>
              <p className="text-[11px] text-muted-foreground mt-1 truncate">
                {n.subjects?.name || n.batches?.title || "General"}
              </p>
              <p className="text-[11px] text-muted-foreground">{n.category?.toUpperCase()} · {formatBytes(n.file_size)}</p>
              <div className="flex gap-1.5 mt-2.5">
                <Button size="sm" variant="secondary" className="flex-1 px-2" disabled={busy === n.id} onClick={() => openNote(n)}>
                  <BookOpen className="size-3 mr-1" /> Read
                </Button>
                <Button size="sm" variant="ghost" className="px-2" title="Download PDF" disabled={busy === n.id} onClick={() => downloadNote(n)}>
      ...
      <Dialog open={!!viewer} onOpenChange={(o) => !o && setViewer(null)}>
        <DialogContent className="max-w-4xl w-[95vw] h-[85vh] flex flex-col p-4">
          <DialogHeader><DialogTitle className="truncate pr-8">{viewer?.title}</DialogTitle></DialogHeader>
          {viewer && <iframe src={viewer.url} title={viewer.title} className="flex-1 w-full rounded-lg border border-border" />}
        </DialogContent>
      </Dialog>
    </div>
  );
